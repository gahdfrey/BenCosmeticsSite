...
const flattenedData = TAB_DATA.map(item => ({
  heading: item.heading,
  title: item.title,
  image: item.image,
  link: item.link,
  fullDescription: item.description.fullDescription,
  keyIngredients: item.description.keyIngredients.join('; '), // Join arrays into a single string
  benefits: item.description.benefits.join('; '),
  targetAudience: item.description.targetAudience,
  howToUse: item.description.howToUse,
}));
...
try {
  ...
  console.log('CSV file has been created successfully: products.csv');
} catch (err) {
